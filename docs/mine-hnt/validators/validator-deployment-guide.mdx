...
- Deploy a virtual server on a cloud service like [Akash Network](/mine-hnt/validators/validator-on-akash), Amazon AWS, Microsoft Azure, Digital Ocean, or Google Cloud Platform (GCP). Or deploy a physical server on which to run the validator. [The recommended hardware and networking requirements can be found here](/mine-hnt/validators/requirements/).
